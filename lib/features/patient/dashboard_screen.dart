...
// ─── Section Prodromes ────────────────────────────────────────
const _knownSymptoms = [
  'Maux de tête', 'Aura visuelle', 'Fatigue', 'Anxiété',
  'Irritabilité', 'Nausées', 'Troubles du sommeil',
  'Sensation de déjà-vu', 'Picotements', 'Troubles de l\'humeur',
];

class _ProdromeSection extends ConsumerWidget {
  final String uid;
  const _ProdromeSection({required this.uid});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final prodromes = ref.watch(prodromeProvider(uid));
          ...
          data: (list) {
              ...
              return Container(
                ...
                child: Row(children: [
                  ...
                  const Expanded(child: Text(
                    'Aucun prodrome enregistré.\nNotez vos signes avant-coureurs.',
                    style: TextStyle(
            ...
            return Column(
              children: list.take(3).map((p) =>
                _ProdromeTile(
                  entry: p,
                  onDelete: () => deleteProdrome(uid, p.id),
                ),
              ).toList(),
            );
          },
  ...
  static void _showAddSheet(BuildContext context, String uid) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _ProdromeSheet(uid: uid),
    );
  }
}

class _ProdromeTile extends StatelessWidget {
  final ProdromeEntry entry;
  final VoidCallback  onDelete;
  const _ProdromeTile({required this.entry, required this.onDelete});

  @override
  Widget build(BuildContext context) {
    final fmt = '${entry.date.day.toString().padLeft(2, '0')}/'
              ...
              onTap: () async {
                final ok = await showDialog<bool>(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    content: const Text('Ce prodrome sera supprimé.'),
                    actions: [
                      TextButton(
...
class _ProdromeSheet extends StatefulWidget {
  final String uid;
  const _ProdromeSheet({required this.uid});
  @override
  State<_ProdromeSheet> createState() => _ProdromeSheetState();
}

class _ProdromeSheetState extends State<_ProdromeSheet> {
  ...
  Future<void> _save() async {
    if (_selected.isEmpty && _noteCtrl.text.trim().isEmpty) return;
    setState(() => _saving = true);
    final entry = ProdromeEntry(
      id:       DateTime.now().millisecondsSinceEpoch.toString(),
      date:     DateTime.now(),
      symptoms: _selected.toList(),
      note:     _noteCtrl.text.trim(),
    );
    await addProdrome(widget.uid, entry);
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            const Text(
              'Signes ressentis avant une éventuelle crise',
              style: TextStyle(
                fontSize: 12, color: AppColors.textSecondary)),
            const SizedBox(height: 16),
